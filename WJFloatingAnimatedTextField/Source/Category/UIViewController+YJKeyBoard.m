...
- (void)setProperty {
    ...
    if (self.yj_currentEditView) {
        currentEditView = self.yj_currentEditView;
    } else {
...
- (void)setYj_currentEditView:(UIView *)yj_currentEditView {
    objc_setAssociatedObject(self, &YjCurrentEditView, yj_currentEditView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (UIView *)yj_currentEditView {
    return objc_getAssociatedObject(self, &YjCurrentEditView);
}
